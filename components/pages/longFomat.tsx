...
const LongsFormat: React.FC = () => {
  const [posts, setPosts] = useState<Post[]>([]);
  const [latestPostLongsFormats, setLatestPostLongsFormat] = useState<Post | null>(null);
  ...
  const fetchLatestPostLongsFormat = async () => {
    try {
      ...
      const data = await response.json();
      setLatestPostLongsFormat(data.length > 0 ? data[0] : null);
    } catch (error) {
  ...
  useEffect(() => {
    fetchPostsLongsFormat(); // Fetch posts
    fetchLatestPostLongsFormat(); // Fetch the latest post
    const intervalId = setInterval(() => {
      fetchLatestPostLongsFormat(); // Refresh the latest post every 1 second
    }, 1000);
  ...
  const renderLatestPostLongsFormat = (post: Post) => (
        ...
        <FlatList
          ListHeaderComponent={
            <View>
              ...
              {!searchQuery && latestPostLongsFormats && (
                <Animatable.View
                  animation="fadeIn"
                  duration={2000}
                  key={key}
                ...
                >
                  ...
                  {renderLatestPostLongsFormat(latestPostLongsFormats)}
                </Animatable.View>
              )}
            </View>
          }
