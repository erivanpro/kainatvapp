...
const Captations: React.FC = () => {
  const [captations, setCaptations] = useState<Post[]>([]);
  const [latestPostCaptations, setLatestPostCaptations] = useState<Post | null>(null);
  ...
  useEffect(() => {
    fetchPostsCaptations();
    fetchLatestPostCaptations();
    const intervalId = setInterval(fetchLatestPostCaptations, 10000); // Refresh every 10 seconds
    return () => clearInterval(intervalId);
  }, []);

  const filterPosts = (query: string) => {
    return captations.filter(
      (post) =>
  ...
  const renderLatestCaptations = (post: Post) => (
        ...
        <FlatList
          ListHeaderComponent={
            <View>
              ...
              {!searchQuery && latestPostCaptations && (
                <Animatable.View
                  animation="fadeIn"
                  duration={2000}
                  key={key}
                ...
                >
                  ...
                  {latestPostCaptations && renderLatestCaptations(latestPostCaptations)}
                </Animatable.View>
              )}
            </View>
          }
