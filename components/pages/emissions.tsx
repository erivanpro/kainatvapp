...
const Emissions: React.FC = () => {
  const [emissions, setEmissions] = useState<Post[]>([]);
  const [latestPostEmissions, setLatestPostEmissions] = useState<Post | null>(null);
  ...
  useEffect(() => {
    fetchPostsEmissions();
    fetchLatestPostEmissions();
    const intervalId = setInterval(fetchLatestPostEmissions, 10000); // Refresh every 10 seconds
  ...
  const renderLatestEmissions = (post: Post) => (
        ...
        <FlatList
          ListHeaderComponent={
            <View>
              ...
              {!searchQuery && latestPostEmissions && (
                <Animatable.View
                  animation="fadeIn"
                  duration={2000}
                  key={key}
                ...
                >
                  ...
                  {latestPostEmissions && renderLatestEmissions(latestPostEmissions)}
                </Animatable.View>
              )}
            </View>
          }
